...
const sqlOwner = process.env.SQL_OWNER;
...
ALTER FUNCTION ${getFncName}(integer) OWNER TO ${sqlOwner};
GRANT EXECUTE ON FUNCTION ${getFncName}(integer) TO ${sqlUser};
`;
...
LANGUAGE plpgsql VOLATILE
                   COST 100;
ALTER FUNCTION ${delFncName}(integer) OWNER TO ${sqlOwner};
GRANT EXECUTE ON FUNCTION ${delFncName}(integer) TO ${sqlUser};`;
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN jsonb_build_object('error','Wrong filter format', 'code', 400);
  END;
...
ALTER FUNCTION ${listFncName}(character varying) OWNER TO ${sqlOwner};
GRANT EXECUTE ON FUNCTION ${listFncName}(character varying) TO ${sqlUser};
`;
...
$BODY$
    LANGUAGE plpgsql VOLATILE
                     COST 100;
ALTER FUNCTION ${saveFncName}(character varying) OWNER TO ${sqlOwner};
GRANT EXECUTE ON FUNCTION ${saveFncName}(character varying) TO ${sqlUser};
`;
